...
@section('title-flex')
    <div>
        @can('close', $training)
            <a href="{{ route('pilot.training.action.close', $training->id) }}"
                onclick="return confirm('Are you sure you want to close your training?')" class="btn btn-danger"><i
                    class="fas fa-xmark"></i> Close my training</a>
        @endcan
    </div>
@endsection
@section('content')
    ...
    @if (
        $training->status < \App\Helpers\TrainingStatus::COMPLETED->value &&
            $training->status != \App\Helpers\TrainingStatus::CLOSED_BY_STUDENT->value)
            ...
            <div class="card shadow mb-2">
                <div class="card-header bg-primary py-3 d-flex flex-row column-gap-3 pe-0">
                    ...
                    @can('create', \App\Models\Task::class)
                        <button class="btn btn-light btn-icon dropdown-toggle" type="button" id="dropdownMenuButton"
                ...
                <div class="card-body">
                    <dl class="copyable">
                        <dt>State</dt>
                        <dd><i
                                class="{{ $statuses[$training->status]['icon'] }} text-{{ $statuses[$training->status]['color'] }}"></i>&ensp;{{ $statuses[$training->status]['text'] }}{{ isset($training->paused_at) ? ' (PAUSED)' : '' }}
                        </dd>
                        ...
                        <dd>
                            ...
                            <button type="button" onclick="navigator.clipboard.writeText('{{ $training->user->id }}')"><i
                                    class="fas fa-copy"></i></button>
                            <a href="https://stats.vatsim.net/stats/{{ $training->user->id }}" target="_blank"
                                title="VATSIM Stats" class="link-btn me-1"><i class="fas fa-chart-simple"></i></button></a>

                        </dd>
                        <dt> Name </dt>
                        <dd class="separator pb-3"><a href="{{ route('user.show', $training->user->id) }}">
                    ...
                    @can('edit', [\App\Models\PilotTraining::class, $training])
                        <a href="{{ route('pilot.training.edit', $training->id) }}" class="btn btn-outline-primary btn-icon"><i
                                class="fas fa-pencil"></i>&nbsp;Edit training</a>
                    @endcan
                </div>
            </div>
                            ...
                            <form action="#" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($activeTrainingInterest)
                                        <div class="alert alert-warning" role="alert">
                                            <i class="fas fa-exclamation-triangle"></i>&nbsp;This training has an active
                                            interest request pending.
                                        </div>
                                    @endif

                                    <label class="form-label" for="trainingStateSelect">Select training state</label>
                                    <select class="form-select"name="status" id="trainingStateSelect"
                                ...
                                <div class="mb-3" id="closedReasonInput" style="display: none">
                                    ...
                                    <input type="text" id="trainingCloseReason" class="form-control" name="closed_reason"
                                        placeholder="{{ $training->closed_reason }}" maxlength="65">
                                </div>

                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="check1" name="paused_at"
                                        {{ $training->paused_at ? 'checked' : '' }}
                                        @if (!Auth::user()->isInstructorOrAbove()) disabled @endif>
                                    <label class="form-check-label" for="check1">
                                        Paused
                                        @if (isset($training->paused_at))
                                            <span
                                                class='badge bg-danger'>{{ \Carbon\Carbon::create($training->paused_at)->diffForHumans(['parts' => 2]) }}</span>
                                        @endif
                                    </label>
                                </div>
                                ...
                                @if (\Auth::user()->isAdmin())
                                    <div class="mb-3">
                                        <label class="form-label" for="assignInstructors">Assigned instructors: <span
                                                class="badge bg-secondary">Ctrl/Cmd+Click</span> to select multiple</label>
                                        <select multiple class="form-select" name="instructors[]" id="assignInstructors">
                                            @foreach ($instructors as $instructor)
                                                <option value="{{ $instructor->id }}"
                                                    {{ $training->instructors->contains($instructor->id) ? 'selected' : '' }}>
                ...
                @can('comment', [\App\Models\PilotTrainingActivity::class, \App\Models\PilotTraining::find($training->id)])
                    <form action="{{ route('pilot.training.activity.comment') }}" method="POST">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="comment" id="activity_comment" class="form-control border"
                                placeholder="Your internal comment ..." maxlength="512">
                ...
                <div class="timeline">
                    <ul class="sessions">
                        @foreach ($activities as $activity)
                            @can('view', [\App\Models\PilotTrainingActivity::class,
                                \App\Models\PilotTraining::find($training->id), $activity->type])
                                <li data-id="{{ $activity->id }}">
                                    <div class="time">
                                        ...
                                        @can('comment', [\App\Models\PilotTrainingActivity::class,
                                            \App\Models\PilotTraining::find($training->id)])
                                            @if ($activity->type == 'COMMENT' && now() <= $activity->created_at->addDays(1))
                                                <button class="btn btn-sm float-end"
                                                    onclick="updateComment({{ $activity->id }}, '{{ $activity->comment }}')"><i
                                    ...
                                    <p>
                                        ...
                                        @if ($activity->type == 'STATUS')
                                            @if (($activity->new_data == -2 || $activity->new_data == -4) && isset($activity->comment))
                                                Status changed from <span
                                                    class="badge text-bg-light">{{ \App\Http\Controllers\PilotTrainingController::$statuses[$activity->old_data]['text'] }}</span>
                                                to <span
                                                    class="badge text-bg-light">{{ \App\Http\Controllers\PilotTrainingController::$statuses[$activity->new_data]['text'] }}</span>
                                        ...
                                        @elseif($activity->type == 'TYPE')
                                            Training type changed from <span
                                                class="badge text-bg-light">{{ \App\Http\Controllers\PilotTrainingController::$types[$activity->old_data]['text'] }}</span>
                                            to <span
                                                class="badge text-bg-light">{{ \App\Http\Controllers\PilotTrainingController::$types[$activity->new_data]['text'] }}</span>
                                        @elseif($activity->type == 'INSTRUCTOR')
                                            @if ($activity->new_data)
                                                <span
                                                    class="badge text-bg-light">{{ \App\Models\User::find($activity->new_data)->name }}</span>
                                                assigned as instructor
                                            @elseif($activity->old_data)
                                                <span
                                                    class="badge text-bg-light">{{ \App\Models\User::find($activity->old_data)->name }}</span>
        ...
        <div class="col-xl-5 col-md-6 col-sm-12 mb-12">
            <div class="card shadow mb-4">
                <div class="card-header bg-primary py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    @if (
                        $training->status >= \App\Helpers\TrainingStatus::PRE_TRAINING->value &&
                            $training->status <= \App\Helpers\TrainingStatus::AWAITING_EXAM->value)
                        @can('create', \App\Models\Exam::class)
                            <a href="{{ route('exam.create.id', ['id' => $training->user->id]) }}"
                                class="btn btn-icon btn-light"><i class="fas fa-plus"></i> Add Exam</a>
                        @endcan
                    @endif
                </div>
            ...
            <div class="card shadow mb-4">
                <div class="card-header bg-primary py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    @if ($training->status >= \App\Helpers\TrainingStatus::AWAITING_EXAM->value)
                        @can('create', \App\Models\Exam::class)
                            <a href="{{ route('exam.practical.create.id', ['id' => $training->user->id]) }}"
                                class="btn btn-icon btn-light"><i class="fas fa-plus"></i> Add Exam</a>
                        @endcan
                    @endif
                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($exam->attachments && $exam->attachments->count() > 0)
                                                    <div>
                                                        <a href="{{ route('exam.object.attachment.show', ['attachment' => $exam->attachments]) }}"
                                                            target="_blank">
                                ...
                                @can('create', [\App\Models\PilotTrainingReport::class, $training])
                                    @if ($training->status >= \App\Helpers\TrainingStatus::PRE_TRAINING->value)
                                        <a class="dropdown-item"
                                            href="{{ route('pilot.training.report.create', ['training' => $training->id]) }}"><i
                                                class="fas fa-file"></i> Training Report</a>
                                    @endif
                                @else
                                    <a href="#" class="dropdown-item disabled"><i
                                            class="fas fa-lock"></i>&nbsp;Training Report</a>
                                @endcan
                            </div>
                        </div>
                    @endif
                                                ...
                                                <div class="card-header p-0">
                                                    <h5 class="mb-0">
                                                        <button class="btn btn-link" type="button" data-bs-toggle="collapse"
                                                            data-bs-target="#{{ $uuid }}" aria-expanded="true">
                                                            <i
                                                                class="fas fa-fw fa-chevron-right me-2"></i>{{ $report->report_date->toEuropeanDate() }}
                                                            <i class="fas fa-scroll ms-2 me-1"></i>
                                                            <span class="d-inline-block align-middle text-truncate"
                                                                style="max-width: 320px;">{{ $report->lesson->name }}</span>
                                                            @if ($report->draft)
                                                ...
                                                <div id="{{ $uuid }}" class="collapse"
                                                    ...
                                                    <div class="card-body">
                                                        <small class="text-muted">
                                                            ...
                                                            {{ isset(\App\Models\User::find($report->written_by_id)->name) ? \App\Models\User::find($report->written_by_id)->name : 'Unknown' }}&emsp;
                                                            @if (isset($report->lesson_id))
                                                                <i class="fas fa-scroll"></i> {{ $report->lesson->name }}
                                                            @endif
                                                            @can('update', $report)
                                                                <a href="{{ route('pilot.training.report.edit', $report->id) }}"
                                                                    class="float-end"><i class="fa fa-pen-square"></i> Edit</a>
                                                            @endcan
                                                        </small>
                                                        ...
                                                        @if ($report->attachments->count() > 0)
                                                            ...
                                                            @foreach ($report->attachments as $attachment)
                                                                <div>
                                                                    <a href="{{ route('pilot.training.object.attachment.show', ['attachment' => $attachment]) }}"
                                                                        target="_blank">
                                                                        <i
                                                                            class="fa fa-file"></i>&nbsp;{{ $attachment->file->name }}
                                                                    </a>
                                                                </div>
                                                            @endforeach
                                                        @endif
                                            ...
                                            <div class="card-header p-0">
                                                <h5 class="mb-0 bg-lightorange">
                                                    <button class="btn btn-link" type="button" data-bs-toggle="collapse"
                                                        data-bs-target="#{{ $uuid }}" aria-expanded="true">
                                                        <i
                                                            class="fas fa-fw fa-chevron-right me-2"></i>{{ $reportModel->examination_date->toEuropeanDate() }}
                                                    </button>
                                                </h5>
                                            </div>
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <small class="text-muted">
                                                        @if (isset($reportModel->position))
                                                            ...
                                                            {{ \App\Models\Position::find($reportModel->position_id)->callsign }}&emsp;
                                                        @endif
                                                        <i class="fas fa-user-edit"></i>
                                                        {{ isset(\App\Models\User::find($reportModel->examiner_id)->name) ? \App\Models\User::find($reportModel->examiner_id)->name : 'Unknown' }}
                                                        @can('delete', [\App\Models\TrainingExamination::class, $reportModel])
                                                            <a class="float-end"
                                                                href="{{ route('training.examination.delete', $reportModel->id) }}"
                                                                onclick="return confirm('Are you sure you want to delete this examination?')"><i
                                                    ...
                                                    @if ($reportModel->attachments->count() > 0)
                                                        @foreach ($reportModel->attachments as $attachment)
                                                            <div>
                                                                <a href="{{ route('training.object.attachment.show', ['attachment' => $attachment]) }}"
                                                                    target="_blank">
                                                                    <i
                                                                        class="fa fa-file"></i>&nbsp;{{ $attachment->file->name }}
                                                                </a>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($interest->confirmed_at)
                                                    <i
                                                        class="fas fa-check text-success"></i>&nbsp;{{ $interest->confirmed_at->toEuropeanDate() }}
    ...
    @foreach ($requestTypes as $requestType)
        @if ($requestType->allowNonVatsimRatings() == true || $requestType->allowNonVatsimRatings() == false)
            @include('pilot.training.parts.taskmodal', [
